...
impl Snake {
    ...
    fn try_teleport(&mut self, mut next_x: isize, mut next_y: isize) -> (isize, isize){
        if next_x < 0 {
            next_x = NUM_COLS as isize -1;
        } else if next_x >= NUM_COLS as isize {
            next_x = 0;
        }
        if next_y < 0 {
            next_y = NUM_ROWS as isize -1;
        } else if next_y >= NUM_ROWS as isize {
            next_y = 0;
        }
        (next_x, next_y)
    }
